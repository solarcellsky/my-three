...
export default {
  ...
  data() {
    return {
      models: [{ name: "assets/z.glb", position: { x: 0, y: 0, z: 0 } }],
    };
  },
  mounted() {
  ...
  methods: {
    ...
    initHelpers() {
      ...
      worldScene.background = new THREE.TextureLoader().load(
        "assets/texture/galaxy.jpg"
      );
      ...
      loader.load(
        ...
        function(gltf) {
          ...
          scene.traverse((node) => {
            ...
            const geometryLine = new THREE.LineSegments(
              geometryEdges,
              edgesMtl
            );
        ...
        child.addEventListener("mouseout", (event) => {
          ...
          worldScene.children.map((g) => {
            if (
              ...
              g.children[0].material.opacity = 0.1;
            }
          });
    ...
    bindSceneEvent(scene) {
      ...
      lableDiv.setAttribute(
        "style",
        "background: rgba(0, 0, 255, .6);color:#efefef;padding: 5px;font-size:10px;margin-top: -1em;border-radius: 3px;padding: 3px 8px;"
      );
      ...
      scene.traverse((child) => {
        if (child.children.length === 0) {
          ...
          child.addEventListener("mouseover", (event) => {
            event.stopPropagation();
            if (!objectsHover.includes(child)) objectsHover.push(child);
            ...
            const position = child.geometry.boundingSphere.center
              ? child.geometry.boundingSphere.center
              : { x: 0, y: 0, z: 0 };
            sceneLabel.position.set(
              position.x + 3,
              position.y + 1.5,
              position.z
            );
    ...
    onProgress(xhr) {
      if (xhr.lengthComputable) {
        ...
        lodingContainer.setAttribute(
          "style",
          "position: absolute;bottom:0;left:0;background: rgba(0, 0, 0, .6);color:#00f3f7;padding: 5px;font-size:10px;"
        );
